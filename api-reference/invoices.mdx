...
`GET /invoices/{id}` returns a single invoice by its document ID. The response payload is **identical** to the `data` block of the [`invoice.created`](/api-reference/webhooks/invoices) webhook — same fields, same conventions — so you can use it to back-fill a missed webhook, reconcile your accounting, or re-fetch a document on demand.
...
For the complete field reference (amounts, lines, snapshot blocks), see the [invoice webhook documentation](/api-reference/webhooks/invoices#data-fields) — the payloads are identical.
