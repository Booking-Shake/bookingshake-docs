...
Invoices are immutable once issued, so there is no `invoice.updated` or `invoice.deleted` event. A correction is always materialized by a new document (typically a credit note), which triggers its own `invoice.created` event.
